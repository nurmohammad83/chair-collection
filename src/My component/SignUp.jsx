...
const SignUp = () => {
	const {signInGoogle,signInGithub,createNewUser}= useContext(ProductContext)
	const navigate= useNavigate()

	const handelwithSignUp =(e)=>{
		...
		.then(res=>{
			...
			toast.warning('Sign Up Field !!', { autoClose: 500})
			console.error(error)
		})
	...
	<div className="flex justify-center space-x-4">
		...
		<button  aria-label="Log in with Twitter" className="p-3 rounded-lg border border-black  hover:border-white">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current">
	...
	<p className="text-xs text-center sm:px-6 dark:text-gray-400">Already You Have an Account ? 
		<Link rel="noopener noreferrer" to='/login' className="underline dark:text-gray-100 font-bold"> Log In</Link>
	</p>
</div>
    );
};
